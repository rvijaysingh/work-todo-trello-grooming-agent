...
### Phase 5 — Run Report
One **"Grooming Report"** card pinned at the top of `Today`, replaced each run (single daily run at 6:30 AM). It has exactly these five sections, in order, each with a count in its header: **"Still overdue and possibly urgent"**, **"Awaiting your decision"** (open proposals), **"Recently archived"** (cards moved to `Agent Archive` this run and cards approaching their 60-day Trello-archive date), **"Done automatically"**, and **"Health stats"** (scratch backlog, hygiene coverage, Tier-2 approval rates, auto-mode flags, Notion override notes). An **"At a glance"** line under the header totals the four action counts.
