...
**Action vocabulary + 3-bullet comments.** Every card comment and report line names its action with one fixed phrase (`vocab.py`): *Merge Duplicates / Move to Archive / Move to Backlog / Recover From Scratch / Rename Card / Fix Due Date / Update Time Label / Increase Time-Sensitivity / Decrease Time-Sensitivity*. Every action comment is exactly three bullets (`vocab.three_bullet`): (1) `Input signals: (1) <name>: <value> …`; (2) `[<Action>]` with from/to; (3) rationale + `Confidence: NN%.`

**Run mode (`run_mode`, supersedes `dry_run`).** `dry_run` (zero writes) / `limited_test` (only the top `limited_test_actions_per_type` highest-confidence actions of each type execute for real, the rest simulated; proposals always real) / `live`. Enforced by a mode-aware `BoardMutator.action()` context manager; SQLite persistence is per-action (real only). The report header states the mode and tags each entry `[REAL]`/`[simulated]`.
...
- **Signals (each carries a value, `signals.py`):** `existing_label_priority` (P0 - High / P1 - Medium), `existing_due_date_matches_card_list`, `spine_workstream_priority_high`, `spine_people_priority` (a People-section priority-raising person the card centers on), `staleness_likelihood` (from card createdAt + the Granola `**Date:**`/`**Source meeting:**` line + task nature; **high vetoes promotion** and routes to `[Move to Archive]`/`[Move to Backlog]`), `implied_task_urgency` (LLM-only — never the sole verified signal), and `no_urgency_markers` (demotions). **Code-enforced promotion vetoes:** high staleness, a Complete/Done/Paused workstream match, and reflection/coaching/career-development cards are never promoted. `[Move to Backlog]` (proposal-only) parks stale-but-strategically-relevant cards on a `Backlog*` list (or suggests creating one).
- **Increase Time-Sensitivity (upward):** scan `Inbox / Triage`, `This Week`, `Next Few Days` for cards with a verified promotion signal (above). Move the card to the fitting list (P0 → Today, P1 → Next Few Days). Where evidence is a hard deadline, the action may add/upgrade the matching "must do" time label.
