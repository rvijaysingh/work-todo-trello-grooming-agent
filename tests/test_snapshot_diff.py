...
def test_diff_manual_label_removal_treated_as_rejection(board, settings, db_path, fresh_board):
    ...
    prev = storage.get_snapshot(db_path, "prev")
    actions = storage.get_actions(db_path, run_id="prev")
    quar = board2.list_by_name(settings.quarantine_list).id
...
def test_diff_quarantine_pullback_records_rejection(board, settings, db_path):
    # Prev merge: survivor dup_exact_a (unchanged), loser quar_pullback parked in quarantine.
    quar = board.list_by_name(settings.quarantine_list).id
    prev_survivor = _card(board, "dup_exact_a")
    prev_loser = _card(board, "quar_pullback", list_id=quar)
    _seed_prev(db_path, [prev_survivor, prev_loser],
               actions=[{"type": "merge", "card_ids": ["dup_exact_a", "quar_pullback"],
                         "payload": {"survivor_id": "dup_exact_a", "loser_ids": ["quar_pullback"]}}])
    prev = storage.get_snapshot(db_path, "prev")
    actions = storage.get_actions(db_path, run_id="prev")

    # Board has quar_pullback in L_today (dragged back out of quarantine).
    rejections = sd.detect_implicit_rejections(prev, board, actions, settings, quar)
